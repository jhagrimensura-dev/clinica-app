...
function TabWhatsApp() {
  ...
  const [configurandoWebhook, setConfigurandoWebhook] = useState(null)
  const [webhookStatus, setWebhookStatus] = useState({})

  const configurarWebhook = async (conta) => {
    setConfigurandoWebhook(conta.id)
    try {
      const res = await fetch('/api/zapi-setup-webhook', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          i: conta.instanciaId,
          t: conta.instanciaToken,
          ct: conta.clientToken || '',
      ...
      setWebhookStatus(s => ({ ...s, [conta.id]: 'erro' }))
    }
          ...
          editando?.id === conta.id ? (
            ...
            <div key={conta.id} className="flex items-center gap-4 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => configurarWebhook(conta)} disabled={configurandoWebhook === conta.id}
                  title="Reconfigura os webhooks de mensagens recebidas e enviadas na Z-API"
                  className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors disabled:opacity-40 border-blue-300 text-blue-600 hover:bg-blue-50">
                  {configurandoWebhook === conta.id
                    ? <svg className="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
                    : webhookStatus[conta.id] === 'ok' ? '✓' : webhookStatus[conta.id] === 'erro' ? '✗' : '🔗'}
                  {webhookStatus[conta.id] === 'ok' ? 'Webhook ok!' : webhookStatus[conta.id] === 'erro' ? 'Erro' : 'Webhook'}
                </button>
...
function EditarConta({ conta, onSalvar, onCancelar }) {
  ...
  return (
    <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl space-y-3">
      ...
      <div>
        <p className="text-xs font-semibold text-gray-500 mb-1">ID da Instância Z-API</p>
        <input value={instanciaId} onChange={e => setInstanciaId(e.target.value)}
          placeholder="Ex: 3F4668F862E81E850C762328E0209A4"
      ...
      <div>
        <p className="text-xs font-semibold text-gray-500 mb-1">Token da Instância Z-API</p>
        <div className="relative">
          <input type={showInstToken ? 'text' : 'password'} value={instanciaToken} onChange={e => setInstanciaToken(e.target.value)}
            placeholder="Ex: 98EF89C1B2987C6883196E5"
      ...
      <div>
        <p className="text-xs font-semibold text-gray-500 mb-1">Security Token (opcional)</p>
        <div className="relative">
          ...
          <div key={m.id} className="flex items-center gap-4 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <button
                ...
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.145.566 4.157 1.55 5.9L0 24l6.335-1.521A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.882a9.88 9.88 0 01-5.031-1.375l-.36-.214-3.733.897.936-3.618-.235-.372A9.878 9.878 0 012.118 12C2.118 6.533 6.533 2.118 12 2.118S21.882 6.533 21.882 12 17.467 21.882 12 21.882z"/></svg>
                WhatsApp
              </button>
              {m.email && (
