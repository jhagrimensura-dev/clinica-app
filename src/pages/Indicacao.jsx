...
function ModalNovoLead({ onClose, onSalvar, ano, mes }) {
  ...
  const handleSalvar = () => {
    ...
    onSalvar({ nome: paciente.trim(), data, responsavel, fonte: 'indicacao', indicadoPor, status, proximoFollowup, obs: obs.trim(), origem: 'indicacao' })
    onClose()
  }
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-7 space-y-5">
        ...
        <div>
          ...
          <input autoFocus type="text" value={paciente} onChange={e => setPaciente(e.target.value)}
        ...
        <div>
          ...
          <textarea value={obs} onChange={e => setObs(e.target.value)}
            ...
            rows={3} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-green-400 resize-none" />
        </div>
        ...
        <div className="flex justify-end gap-3 pt-1">
          ...
          <button onClick={handleSalvar} disabled={!paciente.trim()}
            className="px-6 py-2.5 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-40">
            Criar
          </button>
        </div>
      </div>
...
export default function Indicacao() {
  ...
  const dataFormatada = (data) => { const [, m, d] = data.split('-'); return `${d}/${m}` }
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mt-1">{leads.length} indicações</p>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <div className="flex justify-between items-start mb-1">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mt-1">de {leads.length} indicações</p>
        </div>
      </div>
                ...
                <div key={lead.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">
                      {lead.indicadoPor ? `Indicado por: ${lead.indicadoPor}` : `Follow ${dataFormatada(lead.data)}`}
                    </p>
                  </div>
                  ...
                  <button onClick={() => abrirWhatsApp(lead.telefone)}
                    className={`text-xl flex-shrink-0 ${lead.telefone ? 'opacity-70 hover:opacity-100' : 'opacity-20 cursor-not-allowed'}`}>💬</button>
                    ...
                    {colLeads.length === 0 ? (
                      ...
                      <div key={lead.id} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          {lead.indicadoPor ? `Por: ${lead.indicadoPor}` : dataFormatada(lead.data)}
                        </p>
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <button onClick={() => abrirWhatsApp(lead.telefone)}
                            className={`text-base ${lead.telefone ? 'opacity-70 hover:opacity-100' : 'opacity-20 cursor-not-allowed'}`}>💬</button>
                        </div>
                      </div>
                    ))}
                  </div>
